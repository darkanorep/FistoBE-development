...
//      $model = new Treasury();
    ...
    public static function voucherTransaction($model, $transaction_id, $tag_no, $reason_remarks, $date_now, $reason_id, $status, $voucher_no, $approver, $account_titles, $transaction_type_id, $transaction_type_name)
    {
            ...
            case 6: // Utilities
                ...
                if (isset($fields["service_group"])) {
                    ...
                    foreach ($fields["service_group"] as $services_group) {
                        $new_transaction->service_batches()->create([
...
//                }
                ...
                if ($fields["document"]["payment_type"] == "Partial") {
                    $transactionData["balance_po_ref_amount"] = $balance_po_ref_amount;
                }
    ...
    public static function updateTransaction($transaction_id, $po_total_amount = 0, $request_id, $date_requested, $fields, $balance_po_ref_amount = 0, $changes)
    {
...
//    $currentTransaction->sub_unit = $fields["document"]["sub_unit"]["name"];
    ...
    public static function is_duplicate_prm_multiple($company_id, $supplier_id, $category_id, $period_covered, $cheque_date)
    {
        return $transaction = Transaction::where("company_id", $company_id)
            ->where("supplier_id", $supplier_id)
    ...
    public static function validateTransactionByDateRange($from, $to, $company_id, $department_id, $supplier_id, $location_id, $category, $account_no, $receipt_no, $id = 0)
    {
        $transactions = DB::table("transactions")
...
//    }
    ...
    public static function validatePayroll($payroll_from, $payroll_to, $company_id, $department_id, $location_id, $supplier_id, $payroll_client, $payroll_type, $payroll_category, $payroll_control_no, $id = 0)
    {
        $duplicate_client = [];
...
//            return GenericMethod::resultLaravelFormat("po_group.no", [
    ...
    public static function validateIfUtilityExist($payment_type, $company_id, $supplier_id, $utilities_from, $utilities_to, $utilities_category, $utilities_account_no, $utilities_consumption, $utilities_uom, $utilities_receipt_no)
    {
    ...
    public static function validateIfPayrollExist($payment_type, $company_id, $supplier_id, $payroll_from, $payroll_to, $payroll_client, $payroll_category, $payroll_type)
    {
        $duplicate_client = 0;
    ...
    public static function validateIfPOExists($po_group, $company_id, $id = 0)
    {
        ...
        $transactions = Transaction::with(['po_details' => function ($query) use ($po_nos) {
            $query->whereIn('po_no', $po_nos);
        }])
            ->withTrashed(function ($query) {
                $query->where('deleted_at', '=', '2024-08-28 00:00:00');
            })
        ...
        foreach ($po_group as $k => $v) {
            $po_no = $v['no'];
            $transaction = $transactions->firstWhere('po_details.po_no', $po_no);
