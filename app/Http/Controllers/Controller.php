...
class Controller extends BaseController
{
  ...
  public function validateIfObjectsExist($model, $arrParam, $modelName)
  {
    ...
    if (!empty($unregisteredObjects)) {
      throw new FistoException(
        $modelName . " not registered or inactive.",
        404,
        null,
        $modelName . " IDs: " . implode(",", $unregisteredObjects)
      );
    }
  }
  ...
  public function validateIfObjectsExistByLocationStore($model, $arrParam, $modelName)
  {
    ...
    if (!empty($unregisteredObjects)) {
      throw new FistoException(
        GenericMethod::addAND($unregisteredObjects) . " is not registered.",
        404,
        null,
        collect(["error_field" => "departments"])
      );
    }
  }
  ...
  public function isTaggedArrayModified($inpputedArrayField, $modelObject, $field)
  {
    ...
    $is_tagged_array_modified = count(
      array_merge(array_diff($previousArray, $inpputedArrayField), array_diff($inpputedArrayField, $previousArray))
    );
    ...
    foreach ($duplicate_lines as $line) {
      ...
      if (empty($object[$line]["account_no"]) || empty($object[$line]["category"])) {
        ...
        $excelDuplicates[] = [
          ...
          "description" =>
            $object[$firstDuplicateLine]["account_no"] .
            " with " .
            strtolower($object[$firstDuplicateLine]["category"]) .
            " category has a duplicate in your excel file.",
        ];
      }
    }
  ...
  public function isUnique($model, $modelName, $params, $fields, $id, $per_field = 0)
  {
    ...
    if (count($params) != count($fields)) {
      ...
      $duplicate_params = array_keys(
        array_filter($param_is_exist, function ($param) {
          return $param !== 0;
        })
      );
  ...
  {
    if (
      count($modelNames) != count($tables) ||
      count($tables) != count($fields) ||
      count($modelNames) != count($fields)
    ) {
      throw new FistoException("Model name, field and table counts are not equal.", 202, null, [
