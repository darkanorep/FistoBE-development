...
//      $paginate = (isset($request['paginate']))? $request['paginate']:$paginate = 1;
        ...
        $search = $request->search;
        $paginate = (int)$request->input('paginate', 1);

        $credit_card = CreditCard::with(['utility_categories', 'utility_locations'])
            ->withTrashed()
...
//      $credit_card = CreditCard::withTrashed()->find($id);
  ...
  public function change_status($id){
        ...
        return $this->changeStatus($id, CreditCard::class, 'Credit Card');
//    $status = $request['status'];
